...
class Cash extends WebLoginBase{
	...
	public final function SearchtoCashLog(){
		$this->display('cash/to-cash-log-list.php');
	}
	public final function SearchrechargeLog(){
		$this->display('cash/recharge-log-list.php');
	}
	...
	public final function ajaxToCash(){
		...
		try{
			...
			//throw new Exception(md5($this->user['regTime'].$para['coinpwd']));
			
			if($this->user['coinPassword']!=md5($this->user['regTime'].$para['coinpwd'])) throw new Exception('资金密码不正确');
			unset($para['coinpwd']);
	...
	public final function inRecharge(){
		...
		if($bk = $this->getRow("select * from {$this->prename}sysadmin_bank where id=?",$para['mBankId'])){
			$bl = $this->getRow("select * from {$this->prename}bank_list where id=?",$bk['bankId']);
			if($para['amount']<$bk['rechargeMin'] || $para['amount']>$bk['rechargeMax']) throw new Exception($bl['name'].'单笔最低'.$bk['rechargeMin'].'元，最高'.$bk['rechargeMax'].'元');
		}else{
		...
		if(strtolower($_POST['vcode'])!=$_SESSION[$this->vcodeSessionName]){
			...
			$para['fcoin']=$this->user['fcoin'];
			$para['actionUid']=$this->user['uid'];
			$para['type']=$bk['type'];
			...
			if($bk['rechargeGift']){
				$para['gift'] = $para['amount']*($bk['gift']/100);
			}else{
				$para['gift'] = 0;
			}
