...
class Deposit extends WebLoginBase{
	...
	public final function pull(){
		...
		if($minute>=$deposittime1 && $minute<$deposittime2){ 
		   ...
		   $this->beginTransaction();
		   try{
				$depositCoin=(($deposit*$depositll3*0.01)/1440)*$deposittime3;
				//$CoinTime=$minute/1440;
	...
	public final function indeposit(){
		...
		$this->beginTransaction();
		try{
			$depositStatus =  $this->getValue("select depositStatus from {$this->prename}members where uid={$para['uid']}");
		    ...
		    if(empty($depositStatus)){
			   ...
			   $member_sql="update {$this->prename}members set deposit=deposit+{$para['amount']},depositStatus=1 where uid={$para['uid']}";
			   $this->update($member_sql);		
			   // 帐变记录
				$this->addCoin(array(
					'coin'=>0-$para['amount'],
					'uid'=>$para['uid'],
			   ...
			   $this->commit();
				...
				throw new Exception('请先全部取出您的余额（收益宝），才能再次进行转入（收益宝）！');
			 }
	...
	public final function outdeposit(){
		...
		if($para['amount']!=$this->user['deposit'] ){throw new Exception('转出金额只能等于当前收益宝资金，请重新输入正确数字！');}
		$this->beginTransaction();
		try{
           $sql="update {$this->prename}members set deposit=deposit-{$para['amount']},depositStatus=0 where uid={$para['uid']}";
           $this->update($sql);
