...
class Tip extends WebBase{
	...
	/**
	 *  站内信提示 DAVID2015-05-09
	 */
	public final function getLetterTip(){
		$sql="select id from {$this->prename}letter where flag=0 and IsRead=0 and aId=".$this->user['uid']." order by id desc limit 1";//获取最新一条站内信
		if($data=$this->getCol($sql)){
			if($cookie=$_COOKIE['letter-LetterTip']){
				$cookie=explode(',',$cookie);
				if(!array_diff($data, $cookie)) return array('flag'=>false);
			}
			$this->query("update {$this->prename}letter set flag=1 where id=".$data[0]);
			//$gdata=$this->getRow("select u.username as username,l.title from {$this->prename}letter l left join ssc_members u on u.uid=sId where l.id=".$data[0]);
			//$username=$gdata['username'];
			//$title=$gdata['title'];
			
			$data=implode(',', $data);
			if($data) setcookie('letter-LetterTip', $data);
			return array(
				'flag'=>true,
				'message'=>'您有一条新的站内信通知！请及时查收！'
			);	
		}
	}
	...
	public final function getTKTip(){
		$sql="select id from {$this->prename}member_cash where (state=0 or state=2 or state=4) and isDelete=0 and flag=0 and uid=".$this->user['uid']." order by id desc";
		if($data=$this->getCol($sql)){
