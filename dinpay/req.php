 ...
 **/
 ////////////////////////////////////请求参数//////////////////////////////////////
		...
		//后台通知地址(必填)
		$notify_url = "http://".$_SERVER["HTTP_HOST"]."/dinpay_callback1.php";

		//定单金额（必填）
		$order_amount = $_POST['p3_Amt'];

		//商家定单号(必填)
		$order_no = $_POST['p2_Order'];//rand(1000,9999).time().rand(1000,9999);
		...
		//商品编号(选填)
		$product_code ="onlinepay";

		//商品描述（选填）
		$product_desc ="onlinepay";

		//商品名称（必填）
		$product_name = "onlinepay";

		//端口数量(选填)
		$product_num = "1";

		//页面跳转同步通知地址(选填)
		$return_url = "http://".$_SERVER["HTTP_HOST"]."/dinpay_callback.php";
		...
		//公用业务回传参数（选填）
		@$extra_return_param ="";// $_POST['pa_MP'];

		// 直联通道代码（选填）
		$bank_code = $_POST['issuerId'];
	...
	//if ($merchant_code<>"2000000043"){  echo("<script>location.href='/';</script>");}
	if($return_url != "") {
	...
	**
	...
	if($show_url != "") {
        ...
        //设置密钥
	//$key = "ylf168888888888888888"; // <支付密钥> 注:此处密钥必须与商家后台里的密钥一致
...
正在进入网银支付页面 ...
<form name="dinpayForm" method="post" action="<? echo $gourl;?>"><!-- 注意 非UTF-8编码的商家网站 此地址必须后接编码格式 -->
