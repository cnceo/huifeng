...
exports.cp=[
			...
			try{
				...
				if(m=str.match(reg)){
					return {
						...
						time:m[3],
						//number:m[1],
						number:m1.substr(0,8)+"-"+m1.substr(8,m1.length),
						data:m[2]
					};
				}					
	...
	//{{{
	{
		...
		parse:function(str){
			try{
				//return return getFrom360Web_1(str,6);
				str=str.substr(0,300);
				var reg=/<row expect="(\d+?)" opencode="([\d\,]+?)" opentime="([\d\:\- ]+?)"/; 
  ...
  //}}}
	...
	//{{{
	{
		...
		parse:function(str){
			try{
				return getFromCaileleWeb_1(str,17);
			}catch(err){
				throw('广东快乐十分解析数据不正确');
			}
		}
...
function getFrom360Web_1(str, type){
	str=str.substr(str.indexOf('<em class="red" id="open_issue">'), 380).replace(/[\r\n]+/g,'');
	var reg=/<em.*?>(\d+)<\/em>.*?<\/h3><div.*?><ul.*?><li.*?>(\d+)<\/li> <li.*?>(\d+)<\/li>.*?<li.*?>(\d+)<\/li>.*?<\/ul><\/div>/,
	match=str.match(reg);
	if(!match) throw new Error('数据不正确');
	//console.log(match);
	var number,_number,number2;
	var d = new Date();
	var y = d.getFullYear();
	if(match[1].length==9 || match[1].length==8){number='20'+match[1];}else if(match[1].length==6){number='2016'+match[1];}else{number=match[1];}
	_number=number;
	if(number.length==11){number2=number.replace(/^(\d{8})(\d{3})$/, '$1-$2');}else{number2=number.replace(/^(\d{8})(\d{2})$/, '$1-0$2');_number=number.replace(/^(\d{8})(\d{2})$/, '$10$2');}
	var myDate = new Date();
	...
	try{
		var data={
			type:type,
			time:_number.replace(/^(\d{4})(\d{2})(\d{2})\d{3}/, '$1-$2-$3 ')+mytime,
			number:number2,
			data:match[2]+','+match[3]+','+match[4]
		};
		//console.log(data);
		return data;
	}catch(err){
...
function ffcTime(){//分分彩
   var dat = new Date();
   var year = dat.getFullYear();       //年
   var month = dat.getMonth() + 1;     //月
   var day = dat.getDate();            //日
   var hh = dat.getHours();            //时
   var mm =dat.getMinutes();          //分
   var clock = year + "";
   if(month < 10)
       clock += "0";
   clock += month + "";
   if(day < 10)
       clock += "0";
   clock += day +"";
   var date = String(parseInt(hh)*60+parseInt(mm)+1);
   	for(var i = 0; i< 4; i++){
		if(date.length < 4){
			date = "0"+date;
		}
