	...
	//北京PK10 结束  从北京福利彩票网  彩经网抓取  ---------------------------------------------------------	
{
	...
	
	{
		...
		parse:function(str){
			try{
				return getguangdong11Fromcj(str,6);
			}catch(err){
				throw('彩经网广东11选5解析数据不正确');
			}
		}
	},
...
function getFromXJFLCPWeb(str, type){
	str=str.substr(str.indexOf(' <p>第'), 300).replace(/[\r\n]+/g,'');
    //console.log(str);
	var reg=/<span> (\d+) <\/span>[\s\S].*?<div class=".*?">[\s\S].*?<i>(\d+)<\/i>[\s\S].*?<i>(\d+)<\/i>[\s\S].*?<i>(\d+)<\/i>[\s\S].*?<i>(\d+)<\/i>[\s\S].*?<i>(\d+)<\/i>/,
	match=str.match(reg);
  //  console.log(match[2]);
	...
	try{
		var data={
			...
			data:match[2]+","+match[3]+","+match[4]+","+match[5]+","+match[6]
		};
