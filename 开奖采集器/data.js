...
//		});
		...
		for(var idx = 0; idx < sqls.length; idx++){
			console.log(sqls[idx]);
			client.query(sqls[idx], function(err,result){
				if(err){
					console.log(err);
				}
			});
		}
		var summ = "call summarizeData(" + data.type + ",'" + data.number + "')";
		console.log(summ);
		client.query(summ, function(err,result){
			if(err){
				console.log(err);
			}
		});
...
http.createServer(function(req, res){
	...
	req.on('data', function(_data){
		...
		var msg={},hash=crypto.createHash('md5');
