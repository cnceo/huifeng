...
<table>
	...
	foreach($data['data'] as $var){ ?>
		<tr>
			<td> 
			    <a data-toggle="modal" href="<?=$this->basePath('Record-betInfo', $var['id'])?>" data-target="#betInfo"><?=$var['wjorderId']?></a>
			</td>
            <td>
			<?
			...
			<td style="display: none;"><?=date('m-d H:i:s', $var['actionTime'])?></td>
            ...
            <?php if($var['lotteryNo'] || $var['isDelete']==1 || $var['kjTime']<$this->time || $var['qz_uid']){ ?>
				--
			<?php }else{ ?>
				<a href="<?=$this->basePath('Game-deleteCode', $var['id']) ?>" dataType="json" call="deleteBet" title="是否确定撤单" target="ajax">撤单</a>
			<?php } ?>
            </td>
		</tr>
	<?php } }else{ ?>
    <tr><td colspan="13">暂无投注信息</td></tr>
    <?php } ?>
	</tbody>
</table>
<?php 
	$this->display('inc_page.php',0,$data['total'],$this->pageSize, $this->basePath($this->controller.'-'.$this->action,'',true)."&txpgs={page}&$params");
?>