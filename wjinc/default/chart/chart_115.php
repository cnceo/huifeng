...
<style>
	esun\:*{behavior:url(#default#VML)}
			...
			<div class="search">
				...
				<table width="100%" id="titlemessage" border="0" cellpadding="0" cellspacing="0" style="background:#DDE0E5;">
					<tr>
						...
						<td>
							<a href="/index.php?tnt=crx&type=<?=$type ?>&count=30">最近30期</a>
							<a href="/index.php?tnt=crx&type=<?=$type ?>&count=50">最近50期</a>
							<a href="/index.php?tnt=crx&type=<?=$type ?>&count=100">最近100期</a>
						</td>
						<!-- td>
							<form method="post">
			...
			<?php 
				...
				$data = $this->getRows("select * from (select * from {$this->prename}data where type=$type order by id desc limit $count)x order by x.id desc");
				$allNums = array("01","02","03","04","05","06","07","08","09","10","11");
			?>
...
<div style="position:relative; height: 950px;" id="container">
	<table id="chartsTable" width="100%" cellpadding="0" cellspacing="0" style="position:absolute; top:0; left:0;">
		<tr id="title"><td rowspan="2"><strong>期号</strong></td><td rowspan="2" colspan="5" class="redtext"><strong>开奖号码</strong></td><td colspan="11"><strong>万位</strong></td><td colspan="11"><strong>千位</strong></td><td colspan="11"><strong>百位</strong></td><td colspan="11"><strong>十位</strong></td><td colspan="11"><strong>个位</strong></td></tr>
		<tr id="head">
			<?php 
				for($i = 0; $i < 5; $i++){
					foreach ($allNums as $xn){
						echo "<td class=\"wdh\"><strong>$xn</strong></td>";
					}
				}
			?>
		</tr>
		...
		<tr id="head">
			...
			<td rowspan="2" colspan="5"><strong>开奖号码</strong></td>
			<?php 
				for($i = 0; $i < 5; $i++){
					foreach ($allNums as $xn){
		...
		<tr id="title"><td colspan="11"><strong>万位</strong></td><td colspan="11"><strong>千位</strong></td><td colspan="11"><strong>百位</strong></td><td colspan="11"><strong>十位</strong></td><td colspan="11"><strong>个位</strong></td></tr>
		</table>
	</div>
