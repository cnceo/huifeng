...
?>
	<tr data-code='<?=json_encode($var)?>'>
		<td style="display: none;"> <a data-toggle="modal" href="<?=$this->basePath('Record-betInfo', $var['id'])?>" data-target="#betInfo"><?=$var['wjorderId']?></a></td>
		<td style="display: none;"><?=date('H:i:s', $var['actionTime'])?></td>
		<td><?=$this->types[$var['type']]['shortName']?></td>
		<td style="display: none;"><?=$this->playeds[$var['playedId']]['name'].$this->iff($var['fpEnable'], ' 飞盘', '')?></td>
		<td style="display: none;"><a data-toggle="modal" href="<?=$this->basePath('Record-betInfo', $var['id'])?>" data-target="#betInfo"><font color="#000"><?=$var['actionNo']?></font></a></td>
		<td style="display: none;"><?=$var['shows']?></td>
		<td style="display: none;"><?=$var['beiShu']?>倍</td>
		<td style="display: none;"><?=$var['actionNum']?></td>
		<td style="display: none;"><?=$modes[$var['mode']]?></td>
		...
		<?php if($var['lotteryNo'] || $var['isDelete']==1 || $var['kjTime']<$this->time || $var['qz_uid']){ ?>
            --
        <?php }else{ ?>
             <a target="ajax" call="gameFreshOrdered"  onajax="confirmCancel" dataType="json" href="<?=$this->basePath('Game-deleteCode', $var['id']) ?>"><font color=#000>撤单</font></a>
        <?php } ?>
        </td>
	</tr>
<?php } }else{ ?>
<tr class="no-records">
      <td colspan="11" height="33" align="center">今天没有投注记录</td>
</tr>
<?php } ?>