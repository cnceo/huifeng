...
?>
<span style="float:left;padding:10px 0px 10px 22px;color:#999;line-height: 20px"></span>
<div class="game-btn2">
	<?php
		if($playeds) foreach($playeds as $played){
			...
			if($played['enable']){
	?>
	<div class="ul-li<?=($played['id']==$this->played)?' current':''?>"><a data_id="<?=$played['id']?>" href="<?=$this->basePath('Index-played', $this->type.'/'.$played['id']) ?>"><?=$played['name']?></a></div>
	<? }} ?>
    <div class="clear"></div>
</div>
