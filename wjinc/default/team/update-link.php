...
<form action="<?=$this->basePath('Team-linkUpdateed') ?>" target="ajax" method="post" call="linkDataSubmitCode" onajax="linkDataBeforeSubmitCode" dataType="html">
   ...
   <div class="modal-body" >
    <ul>
	  <li>
	    <em>会员返点：</em><input type="text" name="fanDian" value="<?=$linkData['fanDian']?>" max="<?=$parentData['fanDian']?>" min="0" fanDianDiff=<?=$this->settings['fanDianDiff']?> ><span style="color:#999">不能高于<?=$parentData['fanDian']?>%</span>
	  </li>
	  <li>
	    <em>不定返点：</em><input type="text" name="fanDianBdw" value="<?=$linkData['fanDianBdw']?>" max="<?=$parentData['fanDianBdw']?>" min="0"/><span style="color:#999">不能高于<?=$parentData['fanDianBdw']?>%</span>
	  </li>
	   <li>
