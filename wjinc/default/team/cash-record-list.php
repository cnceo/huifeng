<?php
	...
	if($_GET['username'] && $_GET['username']!='用户名'){
		...
		switch($_GET['type']=intval($_GET['type'])){
			...
			case 2:
				//直属下线
				if(!$_GET['uid']=intval($_GET['uid'])) $_GET['uid']=$this->user['uid'];
...
<table width="100%" class='table_b'>
	...
	<?php } }else{ ?>
    <tr><td colspan="6">所选条件暂无提现记录！</td></tr>
    <?php } ?>
	</tbody>
</table>
<?php 
	$this->display('inc_page.php',0,$data['total'],$this->pageSize, $this->basePath('Team-searchCashRecord','',true).'&txpgs={page}&'.$params);
?>