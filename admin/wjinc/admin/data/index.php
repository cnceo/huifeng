<?php
	...
	$sqlAmount="select sum(amount) betAmount, sum(b.bonus) zjAmount, sum(b.fanDianAmount) fanDianAmount from ssc_bets b where type={$this->type} and b.isDelete=0";
...
// 					$count['fanDianAmount']+=$amountData['fanDianAmount'];
					$count['betAmount']+=$data['betAmount'];
					$count['zjAmount']+=$data['zjAmount'];
					$count['fanDianAmount']+=$data['fanDianAmount'];
			?>
			<tr>
				...
				<td><?=$data['billCount']?></td>
				<td><?=$data['pjed']?></td>
				<td><?=$data['zjCount']?></td>
				<td><?=$data['userCount']?></td>
				<td><?=$this->ifs($data['betAmount'], '--')?></td>
				<td><?=$this->ifs($data['zjAmount'], '--')?></td>
				<td><?=$this->ifs($data['fanDianAmount'], '--')?></td>
				<td><?=$this->ifs(($data['betAmount']-$data['zjAmount']-$data['fanDianAmount']), '--')?></td>
				<td>
					<a href="/admin778899.php/data/add/<?=$this->type?>/<?=$var['actionNo']?>/<?=$dateString.$var['actionTime']?>" target="modal" width="340" title="添加开奖号码" modal="true" button="提交:dataAddCode|取消:defaultCloseModal"><?=$this->iff($data['data'], '修改', '添加') ?></a>
					<?php if($data['data']){ ?>
