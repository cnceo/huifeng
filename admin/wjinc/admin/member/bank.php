...
<?php
	...
	if($para['bankName']){
		...
		$bankName="and b.name like '%{$para['bankName']}%'";
	}
	
	// $this->pageSize=3;

	$sql="select b.name bankName, i.*, u.username userAccount from {$this->prename}member_bank i, {$this->prename}bank_list b, {$this->prename}members u where b.id=i.bankId and i.uid=u.uid and b.isDelete=0 and i.enable=1 $userWhere $bankName";
// 	echo $sql;
...
<input type="hidden" value="<?=$this->user['coin']?>" />
	...
	<?php if($data['data']) foreach($data['data'] as $var){ ?>
		<tr>
			...
			<td><a href="member/bank?bankName=<?=$var['bankName']?>"><?=$var['bankName']?></a></td>
			<td><?=$var['account']?></td>
			<td><?=$var['username']?></td>
			<td><?=$var['countname']?></td>
