<?php
	...
	$sql="select l.* from {$this->prename}member_session l, {$this->prename}manager u where u.uid=l.uid $timeWhere $userWhere $ipWhere $osWhere $browserWhere order by loginTime desc";
 	//echo $sql;
...
<input type="hidden" value="<?=$this->user['username']?>" />
	...
	<?php if($data['data']){
		...
		<tr>
			<td><?=$var['id']?></td>
			<td><a href="manage/loginLog?username=<?=$var['username']?>"><?=$var['username']?></a></td>
			<td><a href="manage/loginLog?ip=<?=long2ip($var['loginIP'])?>"><?=long2ip($var['loginIP'])?></a></td>
			<td><a href="manage/loginLog?browser=<?=$var['browser']?>"><?=$var['browser']?></a></td>
			<td><a href="manage/loginLog?os=<?=$var['os']?>"><?=$var['os']?></a></td>
