...
<input type="hidden" value="<?=$this->user['username']?>" />
	<header>
		...
		<div class="submit_link wz"><input type="submit" value="添加账变" onclick="addCoinLog()" class="alt_btn"></div>
		<form action="/admin778899.php/business/coinLog" class="submit_link wz" target="ajax" call="defaultSearch" dataType="html">
			会员：<input type="text" class="alt_btn" name="username" style="width:60px;" value="<?=$para['username'] ?>"/>&nbsp;&nbsp;
	...
	?>
		<tr>
			<td><a href="business/coinLog?username=<?=$var['username']?>"><?=$var['username']?></a></td>
			<td><a href="business/coinLog?liqType=<?=$var['liqType']?>"><?=$var['info']?></a></td>
            <?php if($var['extfield0'] && in_array($var['liqType'], array(2,3,4,5,6,7,10,11,100,101,102,103,104,105,108))){ ?>
                <td><a target="modal" button="关闭:defaultCloseModal" width="510" title="投注信息" href="/admin778899.php/business/betInfo/<?=$var['extfield0']?>"><?=$this->ifs($var['extfield0'], '--')?></a></td>
                <td><a href="business/coinLog?type=<?=$var['type']?>"><?=$this->iff($var['type'], $this->types[$var['type']]['title'], '--')?></a></td>
