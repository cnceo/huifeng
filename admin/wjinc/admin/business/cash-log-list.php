...
<?php
	...
	// 银行限制
	if($para['bankName']){
		$para['bankName']=wjStrFilter($para['bankName']);
		if(ctype_alnum($para['bankName'])) throw new Exception('用户名包含非法字符,请重新输入');
		$bankNameWhere=" and b.name like '%{$para['bankName']}%'";
	}
	
	// 账号限制
	if($para['account']){
		$para['account']=wjStrFilter($para['account']);
		if(!ctype_alnum($para['account'])) throw new Exception('用户名包含非法字符,请重新输入');
		$accountWhere=" and d.account like '%{$para['account']}%'";
	}
	...
	$sql="select b.name bankName, c.*, u.username userAccount, d.countname from {$this->prename}bank_list b, {$this->prename}member_cash c, {$this->prename}members u,{$this->prename}member_bank d where c.bankId=b.id and c.uid=u.uid and u.uid=d.uid and c.state<5 and b.isDelete=0 and c.isDelete=0  $timeWhere $userWhere $bankNameWhere $accountWhere order by c.id desc";
	//echo $sql;
...
<?php if($data['data']) foreach($data['data'] as $var){ ?>
    <tr>
        <td><?=$var['uid']?></td>
        <td><a href="business/cashLogList?username=<?=$var['userAccount']?>"><?=$var['userAccount']?></a></td>
        <td><?=$var['amount']?></td>
        <td><a href="business/cashLogList?bankName=<?=$var['bankName']?>"><?=$var['bankName']?></a></td>
        <td><?=$var['username']?></td>
        <td><a href="business/cashLogList?account=<?=$var['account']?>"><?=$var['account']?></a></td>
		<td><?=$var['countname']?></td>
...
</table><tr><span style="font-size:15px;color:#FF0000;margin-left:540px;line-height:40px">本次统计提款总额：<?=$this->iff($amount,$amount,0)?>元</span></tr>
<footer>
<?php
