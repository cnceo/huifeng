<?php
	...
	$sql="select b.home, b.name, u.id, u.account, u.username from {$this->prename}sysadmin_bank u, {$this->prename}bank_list b where u.bankId=b.id and b.isDelete=0 and u.admin=1";
...
<?php if($data['data']) foreach($data['data'] as $var){ ?>
    <tr>
        ...
        <td>
		<?php if($var['mBankId']==0){ ?>
		   <a href="#">后台充值</a>
		  <?php }else{ ?>
		<a href="<?=$bank[$var['mBankId']]['home']?>" title="银行帐号：<?=$bank[$var['mBankId']]['account']?>，开户名：<?=$bank[$var['mBankId']]['username']?>" target="_blank"><?=$bank[$var['mBankId']]['name']?></a>
		<?php } ?>
		</td>
        ...
        <td>
            <?php if(!$var['state']){ ?>
            <a href="/admin778899.php/business/rechargeActionModal/<?=$var['id']?>" target="modal"  width="420" title="编辑用户" button="确定:beforeDealRecharge|取消:defaultCloseModal">到帐处理</a>
...
</table><tr><span style="font-size:15px;color:#FF0000;margin-left:540px;line-height:40px">本次统计充值总额：<?=$this->iff($amount,$amount,0)?>元</span></tr>
<footer>
    <?php
