...
<article class="module width_full">
	...
	<tbody> 
		<tr> 
			...
			<td><?=$this->getValue("select count(distinct uid) from {$this->prename}member_session where isOnLine=1 and {$this->time}-accessTime<{$GLOBALS['conf']['member']['sessionTime']} and uid in (select uid from {$this->prename}members where admin=0)")?></td> 
...
<article class="module width_full">
	...
	<?php
		$sql="select type, sum(beiShu*mode*actionNum*(fpEnable+1)) amount from {$this->prename}bets where lotteryNo!='' group by type";
...
<article class="module width_full">
	...
	<div class="module_content">
	<?php
		$sql="select playedId, sum(beiShu*mode*actionNum*(fpEnable+1)) amount,sum(actionNum) actionNumA from {$this->prename}bets where lotteryNo!='' group by playedId";
