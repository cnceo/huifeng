...
class Commission extends AdminBase{
	...
	public final function updateCommStatus(){
		...
		if(!isset($_GET['commStatusName'])) throw new Exception('获取的参数出错！');
		$commStatusName = $_GET['commStatusName'];
		if($commStatusName=='conCommStatus'){
		   $sql = "update {$this->prename}members set conCommStatus=0";
		}else{
		   $sql = "update {$this->prename}members set lossCommStatus=0";
		}
		$this->update($sql);
		return '已成功更新所有会员发放状态为未领取！';		
	}
	/*
	* DAVID  UPDATE 2015-05-10 发放昨日消费佣金 
	*/
	public final function conComSingle($uid){
		...
		//$toTime = time();
		// 加载系统设置
		// and betAmount > ".floatval($this->settings['conCommissionBase'])."
		$this->getSystemSettings();
		//echo floatval($this->settings['conCommissionBase']);
		//exit;
		$sql="select u.username, u.coin, u.uid, u.type, u.parentId, sum(b.mode * b.beiShu * b.actionNum) betAmount from ssc_members u left join ssc_bets b on u.uid=b.uid and b.isDelete=0 and b.actionTime between $fromTime and $toTime where 1 and u.uid={$uid} group by u.uid";
		...
		try{
			...
			if(floatval($this->settings['conCommissionBase']) > $betAmount && floatval($this->settings['conCommissionBase']) <= $betAmount){
			//执行消费佣金第一规则
	...
	public final function lossComSingle($uid){
		...
		//exit;
			...
			try{
				...
				if(floatval($this->settings['lossCommissionBase']) > abs($lossAmount) && floatval($this->settings['lossCommissionBase']) <= abs($lossAmount)){
			//执行亏损佣金第一规则
			//$sql="select parentId from {$this->prename}members where `uid`=?";
			$log=array(
				'liqType'=>56,
