...
class Manage extends AdminBase{
	...
	public final function addManager(){
	  $regTime=$this->time;
		...
		if($this->getValue("select uid from {$this->prename}manager where username=?", $para['username'])) throw new Exception('用户名已经存在');
		
		$para['password']=md5($regTime.$para['password']);
		if($this->insertRow($this->prename .'manager', $para)){
	...
	public final function deleteManager($uid){
		...
		$sql="update {$this->prename}manager set isDelete=1 where uid=$uid";
		if($this->delete($sql)){
			$manageName=$this->getValue("select username from {$this->prename}manager where uid=?", $uid);
	...
	public final function clearManager($uid){
		$userName=$this->getValue("select username from {$this->prename}manager where uid=?", $uid);
		$sql="call delManager($uid)";
		if($this->update($sql)){
	...
	public final function changeManagerPwd($uid){
		if(!$uid=intval($uid)) throw new Exception('参数出错');
		$manageData=$this->getRow("select password,regTime,username from {$this->prename}manager where uid=$uid");
		$password=$manageData['password'];
		if(!$password) throw new Exception('用户不存在');
		
		$para=$_POST;
		$para['oldpwd']=md5($manageData['regTime'].$para['oldpwd']);
		if($para['oldpwd']!=$password) throw new Exception('原密码不正确');
		
		unset($para['oldpwd']);
		$para['password']=md5($manageData['regTime'].$para['password']);
		if($para['password']==$password) throw new Exception('密码没有变化');
		if($this->updateRows($this->prename .'manager', $para, 'uid='.$uid)){
