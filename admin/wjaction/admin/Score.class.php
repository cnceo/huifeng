...
class Score extends AdminBase{
	...
	public final function activities(){
		$this->display('score/activities.php');
	}

	public final function activityModal($id = null, $flag){
		if($id) $id = intval($id);
		$this->display('modal/activity-modal.php', 0, $id, $flag);
	}
	public final function switchActivityStatus($id){
		if(!$id=intval($id)) throw new Exception('参数出错');
		$sql="update {$this->prename}activity set enable=not enable where id=$id";
		if($this->update($sql)){
// 			$chikaren=$this->getValue("select username from {$this->prename}activity where uid=?",$uid);
// 			$this->addLog(11,$this->adminLogType[11].'[开关操作ID:'.$id.']',$id,$chikaren);
	...
	public final function deleteActivity($id){
		if(!$sid = intval($id)) throw new Exception('参数错误！');
		$sql="delete from {$this->prename}activity where id=$sid";
		if($this->delete($sql)){
// 			$this->addLog(11,$this->adminLogType[11].'[删除ID:'.$sid.']');
			return '已经删除';
		}else{
			throw new Exception('未知错误');
		}
	...
	public final function addActivity(){
		...
		$para['des'] = $_POST['des'];
		$para['start'] = strtotime($_POST['start']);
		$para['stop'] = $_POST['stop']==0?0:strtotime($_POST['stop']);
		$para['enable'] = $_POST['enable'];
		
		$para['isDelete'] = 0;
	
		if($this->insertRow($this->prename .'activity',$para)){
			return '添加成功';
		}else{
			throw new Exception('添加失败');
		}
	}
	public final function updateActivity($id){
		if(!$sid = intval($id)) throw new Exception('参数错误！');
		...
		if($this->updateRows($this->prename .'activity',$para,'id='.$sid)){
			return '操作成功';
		}else{
