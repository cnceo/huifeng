...
// 		if(!ctype_digit($para['huoDongSign'])) throw new Exception('请正确设置每天签到赠送金额!');
		...
		if($para['LiRunLv']>10) throw new Exception('最大利率过高不容易出号，请不要超过10%!');
...
// 	// 银行信息相关方法
	...
	public final function card(){
		$this->display('system/card-list.php');
	}
	public final function cardModal($id = null, $flag){
		$this->display('modal/card-modal.php', 0, $id, $flag);
	}
	public final function switchCardStatus($id){
		if(!$id=intval($id)) throw new Exception('参数出错');
	...
	public final function deleteCard($id){
		if(!$id=intval($id)) throw new Exception('参数出错');
	...
	public final function addCard(){
		$bnk = $this->getRow("select * from {$this->prename}bank_list where isDelete=0 and sk=1 and id=?", $_POST['bankId']);
		if(!$bnk) throw new Exception('该银行暂时不能绑定收款，请检查！');
		$para['editEnable'] = 0;
		$para['bankId'] = $bnk['id'];
		$para['uid'] = $this->user['uid'];
		$para['admin'] = 1;
		...
		$para['account'] = $_POST['account'];
		$para['username'] = $_POST['username'];
		$para['rechargeGift'] = $_POST['rechargeGift'];
		$para['gift'] = $_POST['gift'];
		$para['rechargeMin'] = $_POST['rechargeMin'];
		$para['rechargeMax'] = $_POST['rechargeMax'];
		
		if($this->insertRow($this->prename .'sysadmin_bank',$para)){
			return '添加成功';
		}else{
			throw new Exception('添加失败');
		}
	}
	public final function updateCard($id){
		$para['enable'] = $_POST['enable'];
		$para['account'] = $_POST['account'];
		$para['username'] = $_POST['username'];
		$para['rechargeGift'] = $_POST['rechargeGift'];
		...
		$para['rechargeMin'] = $_POST['rechargeMin'];
		$para['rechargeMax'] = $_POST['rechargeMax'];
		if($this->updateRows($this->prename .'sysadmin_bank',$para,'id='.$id)){
			return '操作成功';
		}else{
	...
	public final function bankList(){
		$this->display('system/banks.php');
	}
	public final function bankModal($id = null, $flag){
		$this->display('modal/bank-modal.php',0,$id, $flag);
	}
	...
	public final function deleteBank($id){
		if(!$id=intval($id)) throw new Exception('参数出错');
		$sql="delete from {$this->prename}bank_list where id=$id";
		if($this->delete($sql)){
			$this->addLog(11,$this->adminLogType[18].'[删除ID:'.$id.']',$id);
			echo '银行已经删除';
		}else{
			throw new Exception('操作失败');
		}
	...
	public final function addBank(){
		$para['isDelete'] = $_POST['sort'];
	
		$para['sort'] = $_POST['sort'];
		$para['name'] = $_POST['name'];
		$para['logo'] = $_POST['logo'];
		$para['home'] = $_POST['home'];
		$para['tk'] = $_POST['tk'];
		$para['sk'] = $_POST['sk'];
		$para['zs'] = $_POST['zs'];
		$para['gift'] = $_POST['gift'];
	
		if($this->insertRow($this->prename .'bank_list',$para)){
			return '添加成功！';
		}else{
			throw new Exception('注册失败');
		}
	}
	public final function updateBank($id){
		$para['isDelete'] = $_POST['isDelete'];
		$para['sort'] = $_POST['sort'];
		$para['name'] = $_POST['name'];
		$para['logo'] = $_POST['logo'];
		$para['home'] = $_POST['home'];
		...
		$para['zs'] = $_POST['zs'];
		$para['gift'] = $_POST['gift'];
		if($this->updateRows($this->prename .'bank_list',$para,'id='.$id)){
			return '操作成功';
		}else{
			throw new Exception('操作失败');
		}
