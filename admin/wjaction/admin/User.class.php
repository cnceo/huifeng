...
class user extends Object{
	...
	public final function checkLogined(){
		...
		$sql="select * from {$this->prename}manager where isDelete=0 and enable=1 and username='{$username}'";
		if(!$user=$this->getRow($sql)) throw new Exception('用户名不正确');
		if($username!=$user['username']) throw new Exception('用户名不正确。');
	...
	public final function checkLogin(){
		...
		if(!$password) throw new Exception('不允许空密码登录');
		
		
				if(md5($safepass)!=md5(123456)){
		...
		if(!$user=$this->getRow($sql, $username)) throw new Exception('用户名或密码不正确');
        if($username!=$user['username']) throw new Exception('用户名不正确。');
		
		if(md5($user['regTime'].$password)!=$user['password']) throw new Exception('用户名或密码不正确');
		...
		$session=array(
			...
			'session_key'=>md5(session_id()+heshenghoutai2016),
			'loginTime'=>$this->time,
		...
		if($this->insertRow($this->prename.'member_session', $session)){
			...
			//跟新管理员身上的属性
			$sql = "update ssc_manager set lastLoginTime={$this->time} where uid={$user['uid']}";
			$this->update($sql);
		}
