...
var timers={};		// 任务记时器列表
var encrypt_key='9dcd2c9fc65044d91d17c0cb582b2381';
...
function getLiRuncount(){
	var client=createMySQLClient();
	client.query("select value from xy_params where name='LiRuncount'", function(err, data){
		if(err){
			LiRuncount=0;
		}else{
			data.forEach(function(v){
...
function getplaycount(data){
	var client=createMySQLClient();
	client.query("select count(distinct(username)) AS s from xy_bets where actionNo=?", [data.number], function(err, data){
		if(err){
			playcount=0;
		}else{
			data.forEach(function(v){
...
function liRunData(data, conf){
	...
	client.query("select actionNum,playedId,actionData,weiShu,mode,beiShu,bonusProp from xy_bets where isDelete=0 and lotteryNo='' and type=? and actionNo=?", [data.type, data.number], function(err, bets){
		if(err){
			...
			if(playcount>LiRuncount){
				if(bjAmount*Math.abs(1-LiRunLv/100)<zjAmount){
					restartTask(conf, 1);
				}else{
					submitData(data, conf);
				}
...
function calcJ(data, flag){
	var client=createMySQLClient();
	sql="select id,playedId,actionData,weiShu from xy_bets where isDelete=0 and type=? and actionNo=?";
	...
	client.query(sql, [data.type, data.number], function(err, bets){
		if(err){
			...
			bets.forEach(function(bet){
				...
				sqls.push(client.format(sql, [bet.id, zjCount, data.data, 'baishengruntime']));
