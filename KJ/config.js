			...
			try {
				...
				if (typeof data.data[0].result.result[0].data === 'object') {
					...
					return {
						type: 7,
						time: time,
						number: number.substr(0, 2) !== '20' ? '20' + number.replace(/^(\d{6})(\d{2})$/, '$1-0$2') : number.replace(/^(\d{8})(\d{2})$/, '$1-0$2'),
						data: data,
					};
				}
			} catch(err) {
			...
			try{
				...
				if(m_data && m_phase && m_time){
					return {
						type:6,
						time:m_time[1],
						number:'20' + m_phase[1].replace(/^(\d{6})(\d{2})$/, '$1-0$2'),
						//number:'20'+ m_phase[1],
...

{
	...
	*/

	{
		...
		option:{
			host:"www.meipengyou.org",
			timeout:50000,
			path: '/index.php/bsylcp/xml',
			headers:{
				"User-Agent": "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.0) "
			}
		},
...
function getFromXJFLCPWeb(str, type){
	...
	str=str.substr(str.indexOf('<p>第<span> '), 300).replace(/[\r\n]+/g,'');
         
	var reg=/(\d{10})/,
	numreg=/<i>(\d)<\/i>[\s\D]*<i>(\d)<\/i>[\s\D]*<i>(\d)<\/i>[\s\D]*<i>(\d)<\/i>[\s\D]*<i>(\d)<\/i>/,
	num=str.match(numreg),
	qh=str.match(reg);
	//console.log(num[0]);
	if(!qh) throw new Error('数据不正确');
	try{
		var data={
			type:type,
			time:mytime,
			number:qh[1].replace(/^(\d{8})(\d{2})$/, '$1-$2'),
