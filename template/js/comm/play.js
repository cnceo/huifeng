...
function gameKanJiangDataC(diffTime, actionNo) {
	...
	if (diffTime <= 0) {
		...
		ds.dialog({
			...
			content: "第&nbsp" + thisNo + "&nbsp期投注已截止！<br />点击[确定]，进入下一期。",
			yesText: '确定',
			timeout: 10,
			onyes: function() {
		...
		if (S && h == 0 && m == 6 && s == 0) {
			playVoice('/template/sound/stop-time.mp3', 'stop-time-voice');
		}
		if (h == 0 && m == 0 && s == 0) {
			loadKjData();
		} else {
		...
		success: function(data, textStatus, xhr) {
			if (!data) {
				...
				try {
					...
					playVoice('/template/sound/kai-jiang.mp3', 'kai-jian-voice');
				} catch (err) {
					setTimeout(loadKjData, 5000);
				}
			}
...
function gameMsgAutoTip() {
	...
	if (calcFun && (calcFun = window[calcFun]) && (typeof calcFun == 'function')) {
		try {
			...
			$('#game-tip-dom').html(' 注数：' + obj.actionNum + ' 注 ' +"</br>"+' 金额：' + (gameGetMode() * gameGetBeiShu() * obj.actionNum).round(2) + ' 元');
...
function gameAddCode(code) {
	...
	try {
		...
		$('<td style="display:none">').append('奖返：' + parseFloat(code.bonusProp).round(2) + '-' + parseFloat(code.fanDian).round(1) + '%')).append(
...
function gamePostCode() {
	...
	//DAVID dialog
	var tipString = "<div style='width:300px;'>确定投注第&nbsp;&nbsp;<b>" + actionNo['actionNo'] + "</b>&nbsp;&nbsp;期彩票？";
	tipString += '<br /><div style="overflow-x:auto;overflow-y:auto;height:300px;display:none"><table width="100%"><thead><tr><th>玩法</th><th>号码</th><th style="display:none">注数</th><th style="display:none">倍数</th><th style="display:none">模式</th></tr></thead>';
	$('.touzhu-cont tr').each(function() {
		var $this = $(this);
		tipString += "<tbody><tr><td style='text-align: center;'>" + $('td:eq(0)', $this).text() + "</td><td class='code-list' style='text-align: center;'>" + $('td:eq(1)', $this).text() + "</td><td style='display:none'>" + $('td:eq(2)', $this).data('value') + "</td><td style='display:none'>" + $('td:eq(4)', $this).text() + "</td><td style='display:none'>" + $('td:eq(5)', $this).text() + "</td></tr>";
	});
