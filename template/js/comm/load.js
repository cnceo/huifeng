$(function() {
	...
	if (typeof(TIP) != 'undefined' && TIP) {
		setInterval(function() {
			reloadMemberInfo();
			$.getJSON('/index.php?tnt=tgt', function(tip) {
				if (tip) {
		...
		setInterval(function() {
			reloadMemberInfo();
			$.getJSON('/index.php?tnt=tgc', function(tip) {
				if (tip) {
					if (!tip.flag) return;
					$("<div>").append(tip.message).dialog({
						position: ['right', 'bottom'],
...
function reloadMemberInfo() {
	$('.userInfo').load('/index.php?tnt=iui');
}

//站内信通知
setInterval(function() {
	$.getJSON('/index.php?tnt=tler', function(tip) {
		if (tip) {
			//if (!tip.flag) return;
			$("<div>").append(tip.message).dialog({
				position: ['right', 'bottom'],
